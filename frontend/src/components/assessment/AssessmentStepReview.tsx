...
export function AssessmentStepReview({ formData, patient }: AssessmentStepReviewProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-primary">Revisão e finalização</h2>
        <p className="text-sm text-gray-600">
          Confira os dados antes de salvar. É possível voltar para qualquer etapa para ajustes.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <SummaryCard title="Dobras cutâneas">
          ...
          {formData.skinfoldNotes && <p className="text-xs text-gray-500 mt-2">Obs.: {formData.skinfoldNotes}</p>}
        </SummaryCard>
      </div>
    </div>
  );
